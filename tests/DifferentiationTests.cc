...
TEST_FIXTURE (CartPendulum, ad_UpdateKinematicsADvsFDTest) {
	...
	for (unsigned int joint_id = 1; joint_id < model.mBodies.size(); joint_id++) {
		for (int idirs = 0; idirs < ndirs; ++idirs) {
			CHECK_ARRAY_CLOSE (fd_X_lambda[joint_id][idirs].data(), ad_model.ad_X_lambda[joint_id][idirs].data(), 36, TEST_PREC);
			CHECK_ARRAY_CLOSE (fd_X_base[joint_id][idirs].data(), ad_model.ad_X_base[joint_id][idirs].data(), 36, TEST_PREC);
