...
void checkModelsADvsFD(
  ...
  // derivative check
  CHECK_ARRAY_CLOSE(ad_d_model.d.data(), fd_d_model.d.data(), ad_d_model.u.cols() * ad_model.d.rows(), 1e-7 );
  ...
  for (unsigned idir = 0; idir < ndirs; idir++) {
    for (unsigned i = 0; i < ad_model.IA.size(); i++) {
      CHECK_ARRAY_CLOSE(ad_d_model.IA[i][idir].data(), fd_d_model.IA[i][idir].data(), 36, 1e-6);
    }
  }
  ...
  // derivative check
  CHECK_ARRAY_CLOSE(ad_d_model.u.data(), fd_d_model.u.data(), ad_d_model.u.cols() * ad_model.u.rows(), 1e-7 );
}
