...
void InverseDynamics(
  ...
  for (unsigned int i = 1; i < model.mBodies.size(); i++) {
    ...
    ed_model.v[i].leftCols(ndirs) += ed_model.v_J[i].leftCols(ndirs);
    // nominal evaluation
    ...
    for (unsigned idir = 0; idir < ndirs; idir++) {
      ed_model.c[i].col(idir) = ed_model.c_J[i].col(idir)
        + crossm (model.v[i], ed_model.v_J[i].col(idir))
        + crossm (ed_model.v[i].col(idir), model.v_J[i]);
    }
    // nominal evaluation
    ...
    if (!model.mBodies[i].mIsVirtual) {
      ...
      for (unsigned idir = 0; idir < ndirs; idir++) {
        ed_model.f[i].col(idir)
          ...
          = model.I[i] * ed_model.a[i].col(idir)
          + Math::ED::crossf (
            model.v[i], ed_model.v[i].col(idir),
            Iv, model.I[i] * ed_model.v[i].col(idir)
          );
      }
      // nominal evaluation
