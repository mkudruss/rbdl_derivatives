    ...
    void ad_jcalc (
        ...
        if (model.mJoints[joint_id].mJointType == JointTypeRevoluteX) {
            ...
            for (unsigned int idir = 0; idir < ndirs; ++idir) {
                ad_model.X_J[joint_id][idir] = AD::Xrotx(q[model.mJoints[joint_id].q_index], q_dirs(model.mJoints[joint_id].q_index, idir));
        ...
        if (model.mJoints[joint_id].mJointType == JointTypeRevoluteX) {
            ...
            for (unsigned int idir = 0; idir < ndirs; ++idir) {
                ...
                ad_model.X_lambda[joint_id][idir] =
                        AD::Xrotx (q[model.mJoints[joint_id].q_index], q_dirs(model.mJoints[joint_id].q_index, idir)) * model.X_T[joint_id].toMatrix();
                ad_model.S[joint_id][idir] = SpatialVector::Zero();  // S = [1., 0., 0., 0., 0., 0.]
            }
            // nominal evaluation
